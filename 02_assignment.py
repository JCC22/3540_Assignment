...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    dic_list=list(iot_sensor_points.values())
    temperatures.extend(dic_list)
...
def display_menu(menu):
    ...
    while still_ordering:
        ...
        choice = input('Please pick one item by entering its corresponding number: ')
        if int(choice) in range (1, len(menu)+1):
            order.append(menu[int(choice)-1])
            print_menu(int(choice)-1,menu)
        else:
            if ((int(choice) > len(menu) + 1) or (int(choice) < 1)):
                print('Retry. Please input a valid menu number.')
            else:
                still_ordering = False
                return int(choice),len(menu)

def print_menu(i,menu):
    print(menu[i],'Succefully Done!')
